...
//	result += calcDiffuse(light_positions[1], light_colors[1], norm) * finalTex + calcSpecular(light_positions[1], light_colors[1], norm);
	...
	result += (calcAmbient(light_colors[0]) + (1.0 - shadowCalc(final_shadow_coord, norm, light_positions[0])) * calcDiffuse(light_positions[0], light_colors[0], norm)) * (finalTex + calcSpecular(light_positions[0], norm));
