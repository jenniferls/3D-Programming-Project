...
void main(){
	...
	float angle = dot(-vec3((VIEW_MAT * MODEL_MAT) * gl_in[0].gl_Position), faceNormal); //Calculate the dot product between the face normal and camera view (point is in view space)
	if(angle >= 0.0f){
